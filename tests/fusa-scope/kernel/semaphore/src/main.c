...
/* Test the common semaphore taken by many threads simultaneously,
 ...
 * @verify{@req{359}}
 */
void test_k_sem_take_by_multi_threads(void)
{
	u32_t sem_count;
	 ...
	 * the same priority with thread_3
	 */
	...
	/* Delay 100ms to make thread_4 waiting on common_sem,
	 * then waiting time of thread_4 is shorter than thread_3
	 */
	k_sleep(K_MSEC(100));
	k_sem_give(&high_prio_sem);
	...
	/* Enable the high prio and
	 * long waiting thread(thread_3) to run
	 */
	...
	zassert_true(sem_count == 0U,
			"mid priority thread shouldn't get the sem");
...
void test_k_sem_give_take(void)
{
	 ...
	 * the count of sem_1 will not increase anymore
	 */
	for (int i = 0; i < 5; i++) {
		k_sem_give(&sem_1);
		sem_count = k_sem_count_get(&sem_1);
...
void test_main(void)
{
	...
	k_thread_access_grant(k_current_get(), &simple_sem,
			...
			&low_prio_sem, &mid_prio_sem, &stack_1, &stack_2,
			&stack_3, &stack_4, &thread_1, &thread_2, &thread_3,
			&thread_4, &common_sem);
	...
	ztest_test_suite(test_semaphore_api,
			...
			ztest_user_unit_test(test_k_sem_take_by_multi_threads)
			);
